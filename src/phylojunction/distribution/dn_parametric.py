...
class DnLogNormal(pgm.DistributionPGM):
    ...
    @staticmethod
    def draw_ln(n_draws: int, mean_param: float, sd_param: float, scale: float=1.0, log_space: bool=True):
    ...
    def __init__(self, pars, parent_node_tracker):
    ...
    def generate(self):
        ...
        try:
            ...
            if len(mean_params) == 1 and len(sd_params) == 1:
                return DnLogNormal.draw_ln(self.n_draws * self.n_repl, mean_params[0], sd_params[0], log_space=log_space).tolist()
            ...
            # otherwise, use one mean and sd for each n_repl draws for each simulation
                ...
                for i in range(len(mean_params)):
                    repl = DnLogNormal.draw_ln(self.n_repl, mean_params[i], sd_params[i], log_space=log_space).tolist()
                    sampled_values += repl
...
class DnNormal(pgm.DistributionPGM):
    ...
    @staticmethod
    def draw_normal(n_draws, mean_param, sd_param):
    ...
    def generate(self):
        ...
        try:
            ...
            # use single provided mean and sd for all (n_draws * n_repl) draws
            if len(mean_params) == 1 and len(sd_params) == 1:
                return DnNormal.draw_normal(self.n_draws * self.n_repl, mean_params[0], sd_params[0]).tolist()

            # (PJNormal sould have taken care of checking dimensions)
            # otherwise, use one mean and sd for each n_repl draws for each simulation
            else:
                for i in range(len(mean_params)):
...
class DnExponential(pgm.DistributionPGM):
    ...
    @staticmethod
    def draw_exp(n_draws: int, scale_or_rate_param: float, rate_parameterization: bool=True):
    ...
    # validation of pars happens in phylojunction_grammar
    def __init__(self, pars: ty.List, parent_node_tracker: dict):
        self.param_dict = dict()
        self.n_draws = pars[0]
        self.n_repl = pars[1]
        ...
        print("self.vectorized_params in PJExponential = ")
        print(self.vectorized_params)
    ...
    def generate(self):
        ...
        try:
            ...
            # (PJExponential sould have taken care of checking dimensions)
            # otherwise, use one mean and sd for each n_repl draws for each simulation
            else:
...
class DnGamma(pgm.DistributionPGM):
    ...
    @staticmethod
    def draw_gamma(n_draws, shape_param, scale_or_rate_param, rate_parameterization=False):
    ...
    def generate(self):
        ...
        try:
            ...
            # (PJGamma should have taken care of checking dimensions)
            # otherwise, use one mean and sd for each n_repl draws for each simulation
            else:
...
class DnUnif(pgm.DistributionPGM):
    ...
    @staticmethod
    def draw_unif(n_draws, min_param, max_param):
        """Return sample from log-normal distribution
    ...
    def generate(self):
        ...
        try:
            ...
            if len(min_params) == 1 and len(max_params) == 1:
                return DnUnif.draw_unif(self.n_draws * self.n_repl, min_params[0], max_params[0]).tolist()

            # (PJUniform sould have taken care of checking dimensions)
            # otherwise, use one mean and sd for each n_repl draws for each simulation
            else:
                for i in range(len(min_params)):
                    repl = DnUnif.draw_unif(self.n_repl, min_params[i], max_params[i]).tolist()
...
if __name__ == "__main__":
    ...
    n_draws = 3
    n_repl = 1
    parent_node_tracker = None
    ln2 = DnLogNormal([n_draws, n_repl, [-2, -2.1, -2.2], [0.1, 0.2, 0.15], True], parent_node_tracker)
    ln3 = DnLogNormal([n_draws, n_repl, [-2], [0.1], True], parent_node_tracker)
    ln4 = DnLogNormal([n_draws, n_repl, [-2], [0.1], True], parent_node_tracker)
