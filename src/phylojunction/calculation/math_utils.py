...
def exp_extant_count_bd(birth_rate: float, death_rate: float, tree_age: float, speciation_k: int=1) -> float:
    ...
    Args:
        ...
        speciation_k (int): Extant taxa are counted conditioning on process starting on k-th speciation event. Default: 1.
    ...
    expected_extant_count = k * math.exp( (birth_rate - death_rate) * tree_age )
