...
class PJDetFnGrammar():
    ...
    @classmethod
    def init_return_sse_rate(cls, det_fn_param_dict: ty.Dict[str, str]) -> sseobj.AtomicSSERateParameter:
        return detsse.make_SSEAtomicRate(det_fn_param_dict)

    @classmethod
    def init_return_macroevol_handler(cls, det_fn_param_dict: ty.Dict[str, str]) -> sseobj.MacroEvolEventHandler:
        return detsse.make_MacroEvolEventHandler(det_fn_param_dict)

    @classmethod
    def create_det_fn_obj(cls, det_fn_id: str, det_fn_param_dict: ty.Dict[str, str]) -> ty.Optional[ty.Union[sseobj.AtomicSSERateParameter, sseobj.MacroEvolEventHandler]]:
        # validate input
