...
class TestAnnotateTreeWithSAsFromRoot(unittest.TestCase):
    ...
    def test_node_counting_oneSA_no_spn_survives(self):
        ...
        To see it on icytree: ((sa1:0.0,sp1:1.0)dummy1:1.0,sp2:2.0)root:0.0;
        """
        ...
        extant_sp2 = Node(taxon=Taxon(label="sp2"), label="sp2", edge_length=2.0)
        ...
        tr_sa_no_spn_survives = Tree(seed_node=root_node)

        tr_sa_no_spn_survives.taxon_namespace.add_taxon(dummy_node)
        tr_sa_no_spn_survives.taxon_namespace.add_taxon(sa_node)
        tr_sa_no_spn_survives.taxon_namespace.add_taxon(extant_sp1)
        tr_sa_no_spn_survives.taxon_namespace.add_taxon(extant_sp2)
        ...
        self.assertEqual(ann_tr_sa_no_spn_survives_max_age.n_extant_terminal_nodes, 2, "Count of terminal extant nodes should be 2.")
        ...
        self.assertEqual(ann_tr_sa_no_spn_survives_no_max_age.n_extant_terminal_nodes, 2, "Count of terminal extant nodes should be 2.")
...
if __name__ == "__main__":
    ...
    # or 
    #
    # $ python3 -m unittest tests.data.test_tree_annot_with_sas_from_root.TestAnnotateTreeWithSAsFromRoot.test_node_counting_oneSA_no_spn_survives
