...
class TestFBDTrees(unittest.TestCase):
    ...
    def test_tree_size_sa_count_max_t_fbd(self):
        """
        Test if FBD trees simulated here have similar root ages and number of sampled ancestors
        as trees simulated with FossilSim, startin from the origin
        ...
        print("FossilSim global mean sa count = " + str(statistics.mean(n_sa_mean_maxt_fossilsim)))
        print("\n\nPJ global mean root age = " + str(global_mean_root_age / 100.0))
        print("FossilSim global mean total root age = " + str(statistics.mean(root_ages_mean_maxt_fossilsim)))
        ...
        a_delta = math.ceil(0.07 * exp_count)
        self.assertAlmostEqual(n_sa_ci_overlap_count, exp_count,
                                msg="Mean absolute difference must be 1.96 * (stderr_python + stderr_fossilsim) apart " + str(exp_count) + " (+/- " + str(a_delta) + ") out of 100 times.", delta=a_delta)
        self.assertAlmostEqual(root_age_ci_overlap_count, exp_count,
                                msg="Mean absolute difference must be 1.96 * (stderr_python + stderr_fossilsim) apart " + str(exp_count) + " (+/- " + str(a_delta) + ") out of 100 times.", delta=a_delta)
...
if __name__ == '__main__':
    ...
    # # not state-dependent (just state 0, and no transition)
    # rates_t0_s0 = [ sseobj.MacroevolStateDependentRateParameter(name="lambda", val=1.0, event=sseobj.MacroevolEvent.W_SPECIATION, states=[0,0,0]),
    #                 sseobj.MacroevolStateDependentRateParameter(name="mu", val=1.0, event=sseobj.MacroevolEvent.EXTINCTION, states=[0]),
    #                 sseobj.MacroevolStateDependentRateParameter(name="psi", val=2.0, event=sseobj.MacroevolEvent.ANCESTOR_SAMPLING, states=[0]) ]
