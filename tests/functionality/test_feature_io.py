...
class TestFeatureIO(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.geo_query = pjgeo.GeoFeatureQuery(cls.two_region_geo_coll)

        cls.cb_is_1_requirement_fn = \
    ...
    def test_geo_query_two_regions(self) -> None:
        ...
        self.geo_query.populate_geo_cond_bit_dicts(
            "land_bridge",
            self.cb_is_1_requirement_fn)

        geo_cond_bit_patterns_list = list()
        for k in self.geo_query.geo_cond_bit_dict["land_bridge"]:
            # * unpacks list
        ...
        self.assertEqual(
        self.geo_query.geo_oldest_cond_bit_dict,
            {'land_bridge': [['1', '1'], ['1', '1']]}
        )

        self.assertEqual(
            self.geo_query.get_geo_condition_change_times("land_bridge"),
            [[[], [1.5]], [[1.5], []]]
        )
